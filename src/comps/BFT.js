import React from 'react'

function BFT() {
  return (
    <div className='w-full bg-BFT bg-no-repeat bg-cover '>
        <div className="flex flex-col items-center justify-center p-20 w-full ">
            <div className="text-white text-6xl mt-40 ">
                <h2>|Join the forum|</h2>
            </div>
            <div className="text-white text-2xl w-600 text-center mt-10 p-3">
                <p className='leading-8'>Join the Legend of Arslan community to learn new information from the developers themselves, as well as offer your ideas!</p>
            </div>
            <button className="bg-lime-900 hover:bg-blue-700 text-white text-2xl font-bold mt-20 py-3 px-6 rounded w-72">
                    Join Now
                </button>
        </div>
    </div>
  )
}

export default BFT